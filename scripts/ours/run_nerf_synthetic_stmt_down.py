...
output_dir = "/cluster/work/cvl/jiezcao/jiameng/Scaffold-GS/nerf_ours_down"

dry_run = False
jobs = list(zip(scenes, factors))

def train_scene(gpu, scene, factor):
    get_folder = "/cluster/work/cvl/jiezcao/jiameng/Scaffold-GS/benchmark_stmt_nerf_down"
    ...
    for scale in [8, 4, 2]:
        ...
        cmd = f"python train_two_stage_syn.py -s {pseudo_gt} " \
              f"-m {model_path} -r 1 --white_background --port {2030 + int(gpu)} --load_gaussian {trained_gaussian} " \
              f"--lod 0 --gpu 1 --voxel_size 0.0 --update_init_factor 4 --appearance_dim 0 --ratio 1"
        print(cmd)
        if not dry_run:
            os.system(cmd)
