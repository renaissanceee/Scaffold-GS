...
output_dir = "/cluster/work/cvl/jiezcao/jiameng/Scaffold-GS/benchmark_stmt_nerf_down"
...
def train_scene(gpu, scene, factor):
    cmd = f"python train.py -s {dataset_dir}/{scene} -m {output_dir}/{scene} -r {factor} --eval --white_background --port {6010 + int(gpu)} " \
    ...
    for scale in [8, 4, 2, 1]:
        cmd = f"python render.py -m {output_dir}/{scene} -r {scale} --scale {scale} --iteration 30000 --skip_train"
        print(cmd)
        if not dry_run:
            os.system(cmd)
