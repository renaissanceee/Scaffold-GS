...
def grow_neural_gaussians(viewpoint_camera, pc, xyz, color, opacity, scaling, rot, mask):
    concatenated = torch.cat([xyz, color, opacity, scaling, rot], dim=-1)
    # grid_scaling = pc.get_scaling[mask]# self._scaling还没动呢
    ## get view properties for gaussian
    ob_view = xyz - viewpoint_camera.camera_center
    ...
    cat_local_view = torch.cat([concatenated, ob_view, ob_dist], dim=1)
    # get offset's opacity
    neural_opacity = pc.get_offset_mlp_opacity(cat_local_view)
    neural_opacity = neural_opacity.reshape([-1, 1])
    mask = (neural_opacity > 0.0)
    mask = mask.view(-1)
    opacity = neural_opacity[mask]
    ...
    # concatenated = torch.cat([grid_scaling, xyz], dim=-1)
    concatenated = xyz
    concatenated_repeated = repeat(concatenated, 'n (c) -> (n k) (c)', k=pc.n_grow)

    concatenated_all = torch.cat([concatenated_repeated, color, scale_rot, offsets], dim=-1)
    masked = concatenated_all[mask]
    # scaling_repeat, repeat_xyz, color, scale_rot, offsets = masked.split([6, 3, 3, 7, 3], dim=-1)
    ...
    # post-process offsets to get centers for gaussians
    # offsets = offsets * scaling_repeat[:, :3]
    xyz_grow = repeat_xyz + offsets

    return xyz_grow, color, opacity, scaling, rot, neural_opacity, mask


def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, visible_mask=None,
           retain_grad=False, offset=False):
    """
    Render the scene.
    ...
    if offset:
        ...
        xyz, color, opacity, scaling, rot, neural_opacity, mask = grow_neural_gaussians(viewpoint_camera, pc, xyz, color, opacity, scaling,
                                                                  rot, visible_mask)
    # ---------------------------------------
