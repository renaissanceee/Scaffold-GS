...
def offset_neural_gaussians(viewpoint_camera, pc, xyz, color, opacity, scaling, rot):
    ...
    # color = color.reshape([anchor.shape[0] * pc.n_offsets, 3])  # [mask]
    return xyz, color, opacity, scaling, rot, d_xyz, d_color
...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, visible_mask=None,
    ...
    if offset:
        ...
        xyz, color, opacity, scaling, rot, d_xyz, d_color = offset_neural_gaussians(viewpoint_camera, pc, xyz, color, opacity, scaling,
                                                                    rot)
