...
class GaussianModel:
    ...
    def __init__(self,
        ...
        self.mlp_offset_xyz = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(feat_dim, 3 * self.n_grow),
        ).cuda()
        self.mlp_offset_cov = nn.Sequential(
            nn.Linear(14 + 3 + 1, feat_dim),
            nn.ReLU(True),
            nn.Linear(feat_dim, 7 * self.n_grow),
        ).cuda()
        self.mlp_offset_color = nn.Sequential(
            nn.Linear(14 + 3 + 1, feat_dim),
            nn.ReLU(True),
            nn.Linear(feat_dim, 3 * self.n_grow),
        ).cuda()
        self.mlp_offset_opacity = nn.Sequential(
            nn.Linear(14 + 3 + 1, feat_dim),
            nn.ReLU(True),
            nn.Linear(feat_dim, self.n_grow),
        ).cuda()
        # ----------------------------------------------
    ...
    def eval(self):
        ...
        self.mlp_offset_cov.eval()
        self.mlp_offset_color.eval()
        self.mlp_offset_opacity.eval()
    ...
    def train(self):
        ...
        self.mlp_offset_cov.train()
        self.mlp_offset_color.train()
        self.mlp_offset_opacity.train()
        if self.appearance_dim > 0:
    ...
    @property
    def get_offset_mlp_cov(self):
        return self.mlp_offset_cov

    @property
    def get_offset_mlp_opacity(self):
        return self.mlp_offset_opacity
    ...
    def training_setup(self, training_args):
        ...
        if self.use_feat_bank:
            ...
            l = [
                ...
                {'params': self.mlp_cov.parameters(), 'lr': training_args.mlp_cov_lr_init, "name": "mlp_cov"},
                {'params': self.mlp_color.parameters(), 'lr': training_args.mlp_color_lr_init, "name": "mlp_color"},
                {'params': self.mlp_offset_xyz.parameters(), 'lr': training_args.mlp_offset_xyz_lr_init,
                 "name": "mlp_offset_xyz"},
                {'params': self.mlp_offset_opacity.parameters(), 'lr': training_args.mlp_offset_opacity_lr_init,
                 "name": "mlp_offset_opacity"},
                {'params': self.mlp_offset_cov.parameters(), 'lr': training_args.mlp_cov_lr_init,
                 "name": "mlp_offset_cov"},
                {'params': self.mlp_offset_color.parameters(), 'lr': training_args.mlp_offset_color_lr_init,
                 "name": "mlp_offset_color"},
            ]
        ...
        self.mlp_offset_xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_offset_xyz_lr_init,
                                                               lr_final=training_args.mlp_offset_xyz_lr_final,
                                                               lr_delay_mult=training_args.mlp_offset_xyz_lr_delay_mult,
                                                               max_steps=training_args.mlp_offset_xyz_lr_max_steps)
        self.mlp_offset_cov_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_offset_cov_lr_init,
                                                               lr_final=training_args.mlp_offset_cov_lr_final,
                                                               lr_delay_mult=training_args.mlp_offset_cov_lr_delay_mult,
                                                               max_steps=training_args.mlp_offset_cov_lr_max_steps)
        self.mlp_offset_color_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_offset_color_lr_init,
                                                                 lr_final=training_args.mlp_offset_color_lr_final,
                                                                 lr_delay_mult=training_args.mlp_offset_color_lr_delay_mult,
                                                                 max_steps=training_args.mlp_offset_color_lr_max_steps)
        self.mlp_offset_opacity_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_offset_opacity_lr_init,
                                                                   lr_final=training_args.mlp_offset_opacity_lr_final,
                                                                   lr_delay_mult=training_args.mlp_offset_opacity_lr_delay_mult,
                                                                   max_steps=training_args.mlp_offset_opacity_lr_max_steps)
        if self.use_feat_bank:
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group['lr'] = lr
            if param_group["name"] == "mlp_offset_xyz":
                lr = self.mlp_offset_xyz_scheduler_args(iteration)
                param_group['lr'] = lr
            if param_group["name"] == "mlp_offset_cov":
                lr = self.mlp_offset_cov_scheduler_args(iteration)
                param_group['lr'] = lr
            if param_group["name"] == "mlp_offset_color":
                lr = self.mlp_offset_color_scheduler_args(iteration)
                param_group['lr'] = lr
            if param_group["name"] == "mlp_offset_opacity":
                lr = self.mlp_offset_opacity_scheduler_args(iteration)
                param_group['lr'] = lr
    ...
    def load_ply_sparse_gaussian(self, path):
        ...
        self._anchor_feat = nn.Parameter(
            torch.tensor(anchor_feats, dtype=torch.float, device="cuda").requires_grad_(False))
        self._offset = nn.Parameter(
            torch.tensor(offsets, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(False))
    ...
    def training_statis_grow(self, viewspace_point_tensor, opacity, update_filter, offset_selection_mask,
                             anchor_visible_mask):
        # update opacity stats
        temp_opacity = opacity.clone().view(-1).detach()
        temp_opacity[temp_opacity < 0] = 0
        temp_opacity = temp_opacity.view([-1, self.n_offsets * self.n_grow])
        # import pdb;pdb.set_trace()
        self.opacity_accum[anchor_visible_mask] += temp_opacity.sum(dim=1, keepdim=True)  # 19964, 59210

        # update anchor visiting statis
        self.anchor_demon[anchor_visible_mask] += 1

        # update neural gaussian statis
        anchor_visible_mask = anchor_visible_mask.unsqueeze(dim=1).repeat([1, self.n_offsets * self.n_grow]).view(-1)
        combined_mask = torch.zeros_like(self.offset_gradient_accum, dtype=torch.bool).squeeze(dim=1)
        combined_mask[anchor_visible_mask] = offset_selection_mask
        temp_mask = combined_mask.clone()
        combined_mask[temp_mask] = update_filter
    ...
    def save_mlp_checkpoints(self, path, mode='split'):  # split or unite
        ...
        if mode == 'split':
            ...
            # --------------------
            self.mlp_offset_xyz.eval()
            offset_mlp_xyz = torch.jit.trace(self.mlp_offset_xyz, (torch.rand(1, 14 + 3 + 1).cuda()))
            offset_mlp_xyz.save(os.path.join(path, 'offset_mlp_xyz.pt'))
            self.mlp_offset_xyz.train()

            self.mlp_offset_cov.eval()
            offset_mlp_cov = torch.jit.trace(self.mlp_offset_cov, (torch.rand(1, 14 + 3 + 1).cuda()))
            offset_mlp_cov.save(os.path.join(path, 'offset_mlp_cov.pt'))
            self.mlp_offset_cov.train()

            self.mlp_offset_color.eval()
            offset_mlp_color = torch.jit.trace(self.mlp_offset_color, (torch.rand(1, 14 + 3 + 1).cuda()))
            offset_mlp_color.save(os.path.join(path, 'offset_mlp_color.pt'))
            self.mlp_offset_color.train()

            self.mlp_offset_opacity.eval()
            offset_mlp_opacity = torch.jit.trace(self.mlp_offset_opacity, (torch.rand(1, 14 + 3 + 1).cuda()))
            offset_mlp_opacity.save(os.path.join(path, 'offset_mlp_opacity.pt'))
            self.mlp_offset_opacity.train()
            # --------------------
    ...
    def load_mlp_checkpoints(self, path, mode='split'):  # split or unite
        if mode == 'split':
            ...
            self.mlp_offset_xyz = torch.jit.load(os.path.join(path, 'offset_mlp_xyz.pt')).cuda()
            self.mlp_offset_cov = torch.jit.load(os.path.join(path, 'offset_mlp_cov.pt')).cuda()
            self.mlp_offset_color = torch.jit.load(os.path.join(path, 'offset_mlp_color.pt')).cuda()
            self.mlp_offset_opacity = torch.jit.load(os.path.join(path, 'offset_mlp_opacity.pt')).cuda()
            if self.use_feat_bank:
